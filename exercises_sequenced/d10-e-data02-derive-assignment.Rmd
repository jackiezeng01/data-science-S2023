...
-   Comment on the relative values of `cov_depth` and `cov_diff`.
    -   `cov_depth` is tiny; there's not much variation in the depth,
        compared to its scale.
-   Why is `cov_diff` so large?
    -   `cov_diff` is enormous! This is because the mean difference
        between `depth` and `depth_computed` is small, which causes the
        `cov` to blow up.
