...
df_q1 <- df_generated %>% 
  ...
  summarize(mean = mean(stats)*4, sd = sd(stats)*4)
df_q1
```
...
- Was your sample size $n$ large enough? Why do you say that?
  - Yes, my sample size n was 10000 and the CLT confidence interval I
    generated consistently contained the mean. I also experimented with
    a couple of other size n and found that the lower the n, the larger
