...
-   Since the number of samples for each distinctness matches, it
    couldn't be because of missing data. I suspect Michelson tweaked the
    data afterwards.
...
-   MIchelson's uncertainty is 102 km/s centered on his predicted light
    speed value, which is a uncertainty buffer of 51 km/s on each side
    of the predicted value. The true error is -151.542, meaning
    Michelson's predicted light speed value is higher than the vacuum
    light speed value. The true error is around 3 times of Michelson's
    predicted uncertainty. In other words, Michelson was too certain in
    his prediction.
...
-   Most of the data points for both the real and the simulated data
    fall between 299900 and 300000.

-   For each given day, the real data and the simulated data have the
...
-   The real data has more variability - for any given day, there may be
    data points clustered together or very far spaced apart. The
    simulated data values are more evenly distributed for any given day.
...
-   There is a positive trend relating a rise of temperature to an
    ...
    temperature warps the equipment used to measure the speed of light
...
-   Temperatures from the upper 50s to upper 60s correlate with data
    with distinctness of around 1-2.
