...
- Maximum cases per 100k
  - 3 of the max cases counties start with a fips code of 48, which is
    Texas

  - 4 of the max cases counties start with a fips code of 02, which is
    Alaska
- Maximum deaths per 100k
  - 3 of the max cases counties start with a fips code of 48, which is
    Texas

  - 2 of the max cases counties start with a fips code of 13, which is
    Georgia
...
df_normalized %>%
  ...
  ggtitle("Covid Cases per 100,000 in counties with pop > 1 million") + 
  theme_minimal() +
  labs(
    x = "Date",
...
df_normalized %>%
  ...
  scale_y_log10(labels = scales::label_number_si()) +
  scale_color_discrete(name = "County") +
  ggtitle("Covid Cases per 100,000 in counties with pop < 1000") + 
  theme_minimal() +
  labs(
    x = "Date",
    y = "Cases (per 100,000 persons)"
...
- I compared the covid cases per 100,000 in counties with population
  that is greater than 1 million and the covid cases per 100,000 in
  counties with population that is less than 1000.

- The graph that represents the population that is 2 million is a lot
  smoother because there are more sample points

- The covid cases per 100000 for counties with a population that is less
  than 1000 also tends to be higher, reaching over 10K cases per 100000
  persons
