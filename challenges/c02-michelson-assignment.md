...
- My mean velocity values are different from Michelson’s values
  slightly. The differences in mean velocity are 1.7, 1.5, and 2, for
  distinctness 1, 2, 3, respectively.

- Since the number of samples for each distinctness matches, it couldn’t
  be because of missing data. I suspect Michelson tweaked the data
  afterwards.
...
# Range that Michelson predicted is correct
...
``` r
sprintf("Lower bound: %f", Lower_bound)
```

    ## [1] "Lower bound: 299893.000000"

``` r
sprintf("Upper bound: %f", Upper_bound)
```

    ## [1] "Upper bound: 299995.000000"
...
- MIchelson’s uncertainty is 102 km/s centered on his predicted light
  speed value, which is a uncertainty buffer of 51 km/s on each side of
  the predicted value. The true error is -151.542, meaning Michelson’s
  predicted light speed value is higher than the vacuum light speed
  value. The true error is around 3 times of Michelson’s predicted
  uncertainty. In other words, Michelson was too certain in his
  prediction.
...
- Most of the data points for both the real and the simulated data fall
  between 299900 and 300000.

- For each given day, the real data and the simulated data have the same
  number of data points.

Differences

- Looking at the mean for each day line, the simulated data fluctuates
  less than the real data. The simulated data aligns more closely with
  Michelson’s predicted value. The real data has mean values that fall
  outside the predicted error range on 4 ays while the simulated only
  fell outside the range on 2 days.

- The real data has more variability - for any given day, there may be
  data points clustered together or very far spaced apart. The simulated
  data values are more evenly distributed for any given day.
...
temp vs distinctness, velocity vs temperature
...
df_q2 %>%
  ...
  theme_minimal() +
  labs(
    title = "Lightspeed Velocity vs Temperature of Measurement",
    x = "Temperature (F)",
    y = "Velocity (in Vacuum)"
  )
```
...
- There is a positive trend relating a rise of temperature to an
  increase in the speed of light. This may be because the rise in
  temperature warps the equipment used to measure the speed of light in
  Michelson’s experiment.

``` r
df_q2 %>%
  group_by(Distinctness, Temp) %>%
  summarize(
    n=n(),
  ) %>%
  ggplot(aes(fill=Distinctness, y=n, x=Temp)) + 
    geom_bar(position="stack", stat="identity") + 
    labs(title = "Temperature's affect on Data Distinctness", x = "Temperature (F)", y = "# of Data Points")
```
...
- Temperatures from the upper 50s to upper 60s correlate with data with
  distinctness of around 1-2.
- Temperatures from the 70 to 90 correlate with data with distinctness
  of around 2-3.
