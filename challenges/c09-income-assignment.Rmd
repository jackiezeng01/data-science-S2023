...
-   Document your observations here.
    -   For 12/14 counties, 2-person families have a smaller confidence interval than that of 6-person families.
    -   Typically, if there are more people in the family, the household earns more. But this is not true for all the data
    -   For less populous counties such as Nantucket and Dukes, the confidence interval is larger
-   Can you confidently distinguish between household incomes in Suffolk county? Why or why not?
    -   Not really because the points are really close together and overlapping
...
## TODO: Pivot the data, rename the columns
  ...
  drop_na(category, income_estimate) %>% 
  mutate(income_SE = income_moe/1.645) %>% 
  mutate(income_lo = income_estimate - income_SE*2.576) %>% # Multiplied by the z value
  mutate(income_hi = income_estimate + income_SE*2.576) %>% 
  mutate(income_CV = income_SE/income_estimate)
...
```{r}
# Find outliers
out <- boxplot.stats(df_q8$income_estimate)$out
out_ind <- which(df_q8$income_estimate %in% c(out))
df_outliers <- (df_q8[out_ind, ])
df_outliers
```
...
-   Across the united states, Asian and Native Hawaiian and Other Pacific Islander had the highest median income_estimate. However, they also have a wider 25 to 75 percentile range.
