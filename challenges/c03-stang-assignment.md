...
df_stang_long <-
  ...
  pivot_longer(
    ...
    names_sep = "_",
    col = c("E_00", "E_45", "E_90", "nu_00", "nu_45", "nu_90"),
    values_drop_na = TRUE,
  ) %>%
  filter(E > 0) %>%
...
# How many aluminum alloys are there?
df_q2 <- df_stang_long %>% 
  # distinct(alloy) %>% 
  # distinct(angle) %>% 
  distinct(thick)
...
- Is there “one true value” for the material properties of Aluminum?
  - No, the material properties of Aluminum depends on several
    properties: E, nu, alloy, angle, and thickness
- How many aluminum alloys are in this dataset? How do you know?
  - There is only one type of alloy: al_24st. I know this because I
    performed the dplyr function distinct() on each column.
- What angles were tested?
  - 0, 45, 90
- What thicknesses were tested?
  - 0.022, 0.032, 0.064, 0.081
- How does the angle affect the elastic (E) value?
...
df_q3 <- df_stang_long %>% 
  group_by(thick) %>% 
  ggplot(aes(y=E, x=thick)) + 
    geom_point() + 
  labs(title = "Thickness's effect on E value", x = "Thickness", y = "E value")
df_q3
```
...
- There seems to be no discernible effect of thickness on the value of
  elasticity .
...
- Does this graph support or contradict the claim above?
  - The graph contradicts this claim. For the thickest sample at 0.081,
    the E and nu values are smaller than that of the thinner samples.
