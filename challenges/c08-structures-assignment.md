...
- Is this estimate of the probability of failure trustworthy? Why or why
  not?
  - It may not be the most trustworthy since there are only 25 samples
    for strength that we used.
- Can you confidently conclude that `POF < 0.03`? Why or why not.
  - Yes, as the standard deviation is also 0, meaning all of the values
    are 0.
...
- Assuming your scopus is the probability of failure `POF` defined
  ...
  variability, or both?
  - I think this is induced variability because it’s generated rather
    than measured in real life
...
- Does the confidence interval above account for uncertainty arising
  ...
  - Yes, since we are basing our 10000 generated samples from the 25
...
- Does this estimate have any uncertainty due to *limited physical
  ...
  - Yes, it might not capture the full range of strength values as there
    are limited physical tests
...
- Does the confidence interval above account for uncertainty arising
  from *limited physical tests* (`df_samples`)? Why or why not?
  - Yes, because the estimate_prof() function uses the limited physical
    tests as a base
- Can you confidently conclude that `POF < 0.03`? Why or why not?
  - Yes. Both the Monte Carlo and the CDF approximation say that it is
    less than 0.03
