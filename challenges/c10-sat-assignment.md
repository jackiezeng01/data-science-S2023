...
- What information do we have about these students?
  - Their high school GPA, SAT score, college computer science class
    GPA, overall college GPA values
- What kinds of information *do we not have* about these students?
  - Personal attributes
  - Background (economic, demographic)
  - Extracurriculars
- Based on these missing variables, what possible effects could be
  present in the data that we would have *no way of detecting*?
  - How the background of the student affects the GPA and SAT scores
  - Whether there is any correlation with particular score trends and
...
- What relationship do `univ_GPA` and `both_SAT` exhibit?
  - There is a positive correlation between university GPA and rising
    SAT scores
- What relationship do `univ_GPA` and `high_GPA` exhibit?
  - There is a positive correlation between university GPA and high
    school GPA
...
- To what extent does `univ_GPA` look like a normal distribution?
  - The section on the right looks like a normal distribution
...
- Which correlations are significantly nonzero?
  - Both of these correlations are statistically significant as the p
    value is smaller than 0.05
- Which of `high_GPA` and `both_SAT` seems to be more strongly
  correlated with `univ_GPA`?
  - high_GPA
- How do the results here compare with the visual you created in q2?
  - It matches. high_GPA’s visual in q2 had a steeper slope, which
    reflects the higher correlation estimate we got from this question.
- Based on these results, what can we say about the predictive
  capabilities of both `high_GPA` and `both_SAT` to predict `univ_GPA`?
  - both high_GPA and both_SAT can predict univ_GPA relatively well.
...
- How does your estimate from q5 compare with your estimate from q4?
  - The estimate from q5 (0.7834) is higher than that of q4 (0.6846776)
...
- What is the confidence interval on the coefficient of `both_SAT`? Is
  ...
  - from 0.001715376 to 0.003415381.
  - The estimate is 0.002565378, which is not very different than 0
- By itself, how well does `both_SAT` predict `univ_GPA`?
  - the p value is 1.081652e-11, so I think it predits univ_GPA quite
    well
...
- How well do these models perform, compared to the one you built in q6?
  - The high_GPA performs better
- What is the confidence interval on the coefficient of `both_SAT` when
  including `high_GPA` as a predictor?? Is this coefficient
  significantly different from zero?
  - the CI is -0.0006740696 to 0.001741962.
  - the estimate is 0.0005339463, which is not very different than zero
